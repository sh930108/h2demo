...
/**
 * @ClassName YamlConvertUtil
 * @Description
 * @Author shanghao5
 * @Date 2020/9/17 11:15
 **/
public class YamlConvertUtil {

    private static final Logger logger = LoggerFactory.getLogger(YamlConvertUtil.class);
    ...
    private static void buildMultilayerMap(Map<String, Object> parent, String path,Object value) {
        ...
        if (key.endsWith("]")) {
            ...
            if (keys.length == 1) {
                parent.put(key, stringToObj(value != null?value.toString():null));
            }else{
    ...
    private static void buildMultilayerList(List<Object> parent,String path,Object value) {
        ...
        if (keys.length == 1) {
            parent.add(index,stringToObj(value != null?value.toString():null));
        } else {
            String newpath = path.substring(path.indexOf(".") + 1);
...
//        properties.load(new FileReader(YamlConvertUtil.class.getResource("/").getFile() + "application.properties"));
//
//        //因为Properties类底层继承了Hashtable<Object,Object>类，所以我们可以直接转
//        Map<String,Object> map=(Map)properties;
//
//
//        String yaml = YamlConvertUtil.flattenedMapToYaml(map);
//        System.out.println(yaml);
//
//        String content = FileUtil.readFileToString(YamlConvertUtil.class.getResource("/").getFile() + "configmap.yaml");
//
...
//        Set<String> stringSet = object.stringPropertyNames();
//        List<String> contentList = Lists.newArrayList();
//        stringSet.stream().filter(s->s.startsWith("data.")).forEach(s->{
//            if(StringUtils.isNotBlank(object.getProperty(s))){
//                contentList.add(object.getProperty(s));
//            }
//        });
//        System.out.println(contentList);
////        Map<String, Object> a= YamlConvertUtil.yamlToFlattenedMap(content);
////        System.out.println(a);
////
////        Map<String, Object> b = YamlConvertUtil.yamlToMultilayerMap(content);
////        System.out.println(b);
////
////
////        String c = YamlConvertUtil.multilayerMapToYaml(b);
////        System.out.println(c);
////        String d = YamlConvertUtil.flattenedMapToYaml(a);
